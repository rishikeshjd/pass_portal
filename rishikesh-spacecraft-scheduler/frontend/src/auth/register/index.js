...
function Register() {
  ...
  /*//--Added by me - from here - to get list of users from server
  const url = '/pswd.json'
  ...
  const submitHandler = async (e) => {
    ...
    /*//--Added by me - from here - to Check if user already exists or not
    let userMatched = false;
    for(let i=0;i<cred.length;i++){
      if (inputs.email === cred[i].user){
        userMatched = true;
        break;        
      }     
    }
    if (userMatched) {
      setErrors({ ...errors, error: true, errorText: "Email is already registered! Sign In." });
      return;
    }
    //--Added by me - to here*/
    ...
    try {
      ...
      /*//--Added by me - from here - to add user to user list in server
      //console.log("User to register:", inputs.name, inputs.password, inputs.email);
      authContext.login(myData);
      axios.post('http://localhost:8080/save', {myData}).then(response =>{
